...
def code_inspection(lines):
    ...
    for line in lines:
        ...
        if command.startswith("."):
            if len(args) == 0:
                if not (re.fullmatch(r"\.(data|code)", line, flags=re.IGNORECASE)):
                    ...
                    report += f"Note: Expected names for sections: \".data\", \".code\"\n\n"
                else:
                    current_section = command
            else:
                if not (re.fullmatch(r"\.org +0x[\da-f]{1,8}", line, flags=re.IGNORECASE)):
                    error_counter += 1
                    report += f"Error: Incorrect origin definition in line {line_index}: {line}\n\n"
                else:
                    mem_addr = int(re.search(r"0x[\da-f]{1,8}", line, flags=re.IGNORECASE).group(0), 16)
            continue
        ...
        if command.endswith(":"):
            if current_section == ".data":
                if not (re.fullmatch(
                    ...
                    if args[0] == ".word" and re.fullmatch(r"-?[1-9]\d{0,9}", args[1]) and not (
                            -2 ** 31 <= int(args[1]) <= 2 ** 31 - 1):
                        error_counter += 1
                        report += f"Error: Defined value out of bounds in line {line_index}: {line}\n"
                        report += f"Note: For .word value supposed to be between -2147483648 and 2147483647\n\n"
                    if args[0] == ".string" and not re.fullmatch(r"\".*\", 0",
                                                                 line.replace(command, "").replace(args[0],
                                                                                                   "").strip()):
                        error_counter += 1
                        report += f"Error: Incorrect string definition in line {line_index}: {line}\n"
                        report += f"Note: .string should be defined as: \"<any symbol>\", 0\n\n"
            labels[command[:len(command) - 1]] = mem_addr
            if current_section == ".data" and ".string" in line:
                mem_addr += len(line.replace(command, "").replace(args[0], "").replace(", 0", "").strip()) - 1
        ...
        if current_section == ".data":
            ...
            continue
        mem_addr += 1
        if command not in instructions and command not in branch_instructions and command not in no_operand_instructions and command not in macros:
            error_counter += 1
    ...
    if "_start" not in labels:
        error_counter += 1
        report += f"Error: \"_start\" label is missing\n"
        report += f"Note: \"_start\" label is crucial to define program's entry point\n\n"
...
def translate(lines):
    ...
    for line in lines:
        ...
        if re.fullmatch(
            ...
            if ".word" in line:
                if re.fullmatch(r"0x[\da-f]{1,8}|0|-?[1-9]\d{0,9}", line.split()[2]):
                    ...
                    report += f"Translation Error: Unknown label in line {line_index}: {line}"
                mem_addr += 1
            else:
                code[mem_addr] = f"str{line.replace(command, '').replace(arg, '').strip()[1:-4]}"
                mem_addr += len(line.replace(command, "").replace(arg, "").strip()[1:-4]) + 1
            continue

        if command == '' or command.startswith(".") or command.endswith(":") or command.startswith(";"):
            continue

        if command not in no_operand_instructions \
                and not re.fullmatch(r"-?[1-9]\d*|0|0x[\da-f]{1,8}", re.sub(r"\*?\[|]|\(pc\)", "", arg),
                                     flags=re.IGNORECASE) \
                and re.sub("[*\[\]]", "", arg, 3) not in labels:
            report += f"Translation error: Unknown label \"{arg}\" in line {line_index}\n\n"
            continue
        ...
        if command in instructions and re.sub("[*\[\]]", "", arg, 3) in labels:
            arg = arg.replace(re.sub("[*\[\]]", "", arg, 3), str(labels[re.sub("[*\[\]]", "", arg, 3)]))
